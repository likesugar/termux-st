...
public class TermuxTerminalExtraKeys extends TerminalExtraKeys {
    ...
    public void onTerminalExtraKeyButtonClick(View view, String key, boolean ctrlDown, boolean altDown, boolean shiftDown, boolean fnDown) {
        ...
        } else if ("OPENBROWSER".equals(key)) {
            mActivity.launchBrowser();
        } else if ("CLEARCACHE".equals(key)) {
            new Thread(() -> {
                String msg;
                try {
                    String[] cmd = {"/data/data/com.termux/files/usr/bin/sh", "-c",
                        "PATH=/data/data/com.termux/files/usr/bin clearcache"};
                ...
                String m = msg;
                mActivity.runOnUiThread(() -> Logger.showToast(mActivity, m.isEmpty() ? "缓存清理完成" : m, false));
            }).start();
        } else {
