...
public abstract class TermuxSharedProperties {
    ...
    public boolean isEnforcingCharBasedInput() {
        // 默认开启（等价于 ~/.termux/termux.properties enforce-char-based-input=true）
        return true;
    }

    public boolean shouldExtraKeysTextBeAllCaps() {
        return (boolean) getInternalPropertyValue(TermuxPropertyConstants.KEY_EXTRA_KEYS_TEXT_ALL_CAPS, false);
    }
    ...
    public boolean isUsingFullScreen() {
        // 默认开启（等价于 ~/.termux/termux.properties fullscreen=true）
        return true;
    }

    public boolean isUsingFullScreenWorkAround() {
        // 关闭：本 ROM 上 adjustResize 正常，workaround 会叠加出大段空隙
        return false;
    }
